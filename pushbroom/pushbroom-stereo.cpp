...
PushbroomStereo::PushbroomStereo() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        lockers_[i] = std::unique_lock<std::mutex>(data_mutexes_[i], std::defer_lock);
...
void* PushbroomStereo::WorkerThread(void *x) {
    ...
    std::mutex *data_mutex = statet->data_mutex;
    int thread_number = statet->thread_number;
    std::condition_variable *cv_new_data = statet->cv_new_data;
    std::condition_variable *cv_thread_finish = statet->cv_thread_finish;

    PushbroomStereo *parent = statet->parent;

    std::unique_lock<std::mutex> locker(*data_mutex, std::defer_lock);
 ...
 * @param hitVector a std::vector that we will populate with cv::Point()s
 ...
 */
void PushbroomStereo::ProcessImages(InputArray _leftImage, InputArray _rightImage, std::vector<Point3f> *pointVector3d, std::vector<uchar> *pointColors, std::vector<Point3i> *pointVector2d, PushbroomStereoState state) {
    ...
    std::vector<Point3f> pointVector3dArray[NUM_THREADS+1];
    std::vector<Point3i> pointVector2dArray[NUM_THREADS+1];
    std::vector<uchar> pointColorsArray[NUM_THREADS+1];
...
void PushbroomStereo::RunStereoPushbroomStereo(PushbroomStereoStateThreaded *statet)
{
    ...
    std::vector<Point3f> *pointVector3d = statet->pointVector3d;
    std::vector<Point3i> *pointVector2d = statet->pointVector2d;
    std::vector<uchar> *pointColors = statet->pointColors;
