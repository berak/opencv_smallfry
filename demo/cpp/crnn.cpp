...
//cout << getBuildInformation() <<endl;
    ...
    if (do_chinese) {
        ...
        net = readNet("c:/data/dnn/crnn.onnx");
    ...
    //in -= 0.5;
    //in /= 0.5;
    float scale = in.size[0]*1.0 / 32;
    cout << "scale " << scale << endl;
    int w = int(in.size[1] / scale);
    if (! do_chinese)
        w=100;
    //resize(in,in,Size(w,32));
//    cout << in.type() << in << endl;

    //Mat blob1 = blobFromImage(in);
    Mat blob2 = blobFromImage(in, 1.0/127.5, Size(w,32),Scalar(127),false,false);
    //cout << "in  " << blob.size << " " << Mat(1,100,CV_32F,blob.data) << endl;

    net.setInput(blob2);
    net.dumpToFile("crnn.dot");

    Mat res = net.forward();
    cout << "out " << res.size << endl;

    string alphabet = "0123456789abcdefghijklmnopqrstuvwxyz";

    /*Mat r22 = res.reshape(1, res.size[0]);
    Mat rs; reduce(r22,rs,1,REDUCE_MAX);
    cout << "rs " << rs.size << endl;
    cout << "rs " << rs << endl;
    */
    for (size_t r=0; r<res.size[0]; r++) {
        Mat s = Mat(1, res.size[2], CV_32F, res.ptr<float>(r));
        Point p; double m;
        minMaxLoc(s, 0,&m, 0,&p);
        if (!do_chinese) {
            char c = p.x>0 ? alphabet[p.x-1] : '-';
            cout << r << "\t" << c << "\t" << p.x << "\t" << m << endl;
        } else {
            if (p.x>0)
                cout << p.x-1 << "," << endl;
