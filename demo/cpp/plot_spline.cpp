...
*/
    // plot
    // generate data
    //vector<double> x{0.03,0.1,0.4,0.7,0.8,0.9,1,1.03,1.1,1.4,1.7,1.8,1.9,2};
    //vector<double> y{.0,.62,.5,1.,1,1,1,.0,.62,.5,1.,1,1,1};
    vector<double> x, y;
    float phase=-1.2;
    float off=0.3;
    for (int i=0; i<800; i++)
    {
        x.push_back(phase*3);
        y.push_back(off+cos(7*phase)*sin(3*phase)*2.75);
        phase += 0.005;
    }
    Mat plotMat;//(480,640,CV_8UC3,Scalar::all(0));
