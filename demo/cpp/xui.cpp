...
struct XUI : UI
{
    struct Item
    {
        ...
        void textbox(Mat &img, float lev, float w=0.25f)
        {
            ...
            putText(img,name,Point(r.x+r.width*0.3,y+3),FONT_HERSHEY_SIMPLEX,0.4,Scalar(50,130,50)+c*lev,1.5, CV_AA);
        }
    ...
    struct Slider : Item
    {
        ...
        virtual void draw(Mat &img)
        {
            ...
            circle(img, Point(r.x + (*v), y), r.height/6, c*1.5*focus, 2, CV_AA);
        }
