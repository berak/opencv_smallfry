...
bool DAT_TRACKER::initImpl(const cv::Mat& image, const cv::Rect2d& boundingBox) {
    model = cv::makePtr<DummyModel>();
	cv::Rect box = boundingBox;
	tracker_dat_initialize(image, box);
	return true;
}

bool DAT_TRACKER::updateImpl(const cv::Mat& image, cv::Rect2d& boundingBox) {
	boundingBox = tracker_dat_update(image);
	return true;
}



cv::Ptr<cv::Tracker> createTrackerDAT()
{
	return cv::makePtr<DAT_TRACKER>();
}
