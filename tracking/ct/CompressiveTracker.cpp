#include "CompressiveTracker.hpp"
...
class TrackerCT : public cv::Tracker {
...
public:
    ...
    virtual bool initImpl(const Mat& image, const Rect2d& boundingBox)
    {
        ...
        ct.init(cvt(image),boundingBox);
        return true;
    }
    virtual bool updateImpl(const Mat& image, Rect2d& boundingBox)
    {
        ct.processFrame(cvt(image), boundingBox);
        return true;
    }
...
cv::Ptr<cv::Tracker> createTrackerCT() { return cv::makePtr<TrackerCT>(); }
...
void CompressiveTracker::HaarFeature(const Rect2d& _objectBox, int _numFeature)
...
void CompressiveTracker::sampleRect(const Mat& _image, const Rect2d& _objectBox, float _rInner, float _rOuter, int _maxSampleNum, vector<Rect>& _sampleBox)
...
void CompressiveTracker::sampleRect(const Mat& _image, const Rect2d& _objectBox, float _srw, vector<Rect>& _sampleBox)
...
void CompressiveTracker::init(const Mat& _frame, const Rect2d& _objectBox)
{
...
void CompressiveTracker::processFrame(const Mat& _frame, Rect2d& _objectBox)
{
	// predict
...
int main(int argc, char** argv) {
    ...
    cv::Ptr<cv::Tracker> tracker = createTrackerCT();
